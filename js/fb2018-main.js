...
$.getJSON("https://bucifan-api.azurewebsites.net/osugames")
  .done(function(games){ 
      for(var i=0;i<games.y2017.length;i++){
          $(".schedule2016").append("<div id='gameitem"+i+"' class='gameitem' data-opinit='"+games.y2017[i].OppShort+"' onclick='toggledtls("+i+");'></div><div id='gamedtl"+i+"' class='hidgamedtls' onclick='toggledtls("+i+");'></div> ");
  
          $("#gameitem"+i).append("<span class='schdate'> "+games.y2017[i].date+"</span>");
          $("#gameitem"+i).append("<span class='schteam'> <img src='/img/helmets/"+games.y2017[i].Himg+"' />"+games.y2017[i].Opp+"</span>");
          if(i==11){
              for(var j=0;j<4;j++){ 
